    ...
    private var home: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 12) {
                ...
                Button(state.pip.isRunning ? "关闭 PiP 悬浮" : "开启 PiP 悬浮") { state.pip.isRunning ? state.pip.stop() : state.pip.start() }.buttonStyle(KingButtonStyle(primary: !state.pip.isRunning)).disabled(!state.pip.isSupported)
                Button(state.floatingHUD.isRunning ? "关闭跨 App HUD" : "开启跨 App HUD") { state.floatingHUD.isRunning ? state.floatingHUD.stop() : state.floatingHUD.start(state: state) }.buttonStyle(KingButtonStyle(primary: !state.floatingHUD.isRunning))
...
struct SettingsSheet: View {
    ...
    private var adjustPage: some View { VStack(spacing: 10) { SliderRow(title: "整体 X", value: $state.settings.offsetX, range: -600...600); SliderRow(title: "整体 Y", value: $state.settings.offsetY, range: -600...600); SliderRow(title: "整体间隔", value: $state.settings.mapSpacing, range: -50...100); SliderRow(title: "头像大小", value: $state.settings.avatarScale, range: 0.6...1.8); SliderRow(title: "不透明度", value: $state.settings.opacity, range: 0.3...1); SliderRow(title: "顶栏大小", value: $state.settings.topSize, range: 0.6...1.8); Toggle("显示校准框", isOn: $calibration).tint(KingTheme.green); Button("自动适配当前屏幕") { state.settings.offsetX = 0; state.settings.offsetY = 0; state.settings.mapSpacing = 0; state.saveSettings() }.buttonStyle(KingButtonStyle()) }.padding(14) }
}
